...
MEMORY
    ...
    RAM (wx) : ORIGIN = 0x24000000, LENGTH = (1024 * 1K)
    QSPI : ORIGIN = 2415919104, LENGTH = 268435456 SRAM0 : ORIGIN = 603979776, LENGTH = 1048576 SRAM3 : ORIGIN = 805306368, LENGTH = 65536 SRAM4 : ORIGIN = 805371904, LENGTH = 65536 SRAM5 : ORIGIN = 939524096, LENGTH = 32768 DTCM : ORIGIN = 536870912, LENGTH = 131072 ITCM : ORIGIN = 0, LENGTH = 65536 SDRAM2 : ORIGIN = 3489660928, LENGTH = 16777216
 ...
 *(".kernel_noinit.*")
    ...
    __kernel_ram_end = 0x24000000 + (1024 * 1K);
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    QSPI (NOLOAD) : { __QSPI_start = .; KEEP(*(QSPI)) KEEP(*(QSPI.*)) __QSPI_end = .; } > QSPI __QSPI_size = __QSPI_end - __QSPI_start; __QSPI_load_start = LOADADDR(QSPI); SRAM0 (NOLOAD) : { __SRAM0_start = .; KEEP(*(SRAM0)) KEEP(*(SRAM0.*)) __SRAM0_end = .; } > SRAM0 __SRAM0_size = __SRAM0_end - __SRAM0_start; __SRAM0_load_start = LOADADDR(SRAM0); SRAM3 (NOLOAD) : { __SRAM3_start = .; KEEP(*(SRAM3)) KEEP(*(SRAM3.*)) __SRAM3_end = .; } > SRAM3 __SRAM3_size = __SRAM3_end - __SRAM3_start; __SRAM3_load_start = LOADADDR(SRAM3); SRAM4 (NOLOAD) : { __SRAM4_start = .; KEEP(*(SRAM4)) KEEP(*(SRAM4.*)) __SRAM4_end = .; } > SRAM4 __SRAM4_size = __SRAM4_end - __SRAM4_start; __SRAM4_load_start = LOADADDR(SRAM4); SRAM5 (NOLOAD) : { __SRAM5_start = .; KEEP(*(SRAM5)) KEEP(*(SRAM5.*)) __SRAM5_end = .; } > SRAM5 __SRAM5_size = __SRAM5_end - __SRAM5_start; __SRAM5_load_start = LOADADDR(SRAM5); DTCM (NOLOAD) : { __DTCM_start = .; KEEP(*(DTCM)) KEEP(*(DTCM.*)) __DTCM_end = .; } > DTCM __DTCM_size = __DTCM_end - __DTCM_start; __DTCM_load_start = LOADADDR(DTCM); ITCM (NOLOAD) : { __ITCM_start = .; KEEP(*(ITCM)) KEEP(*(ITCM.*)) __ITCM_end = .; } > ITCM __ITCM_size = __ITCM_end - __ITCM_start; __ITCM_load_start = LOADADDR(ITCM); SDRAM2 (NOLOAD) : { __SDRAM2_start = .; KEEP(*(SDRAM2)) KEEP(*(SDRAM2.*)) __SDRAM2_end = .; } > SDRAM2 __SDRAM2_size = __SDRAM2_end - __SDRAM2_start; __SDRAM2_load_start = LOADADDR(SDRAM2);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
